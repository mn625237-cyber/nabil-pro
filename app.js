...
async function getAuthHeaders() {
  try {
    ...
    return { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'x-api-key': API_SECRET };
  } catch(e) {
...
async function subscribeFCM() {
  ...
  try {
    ...
    try {
      await fetch('/api/notify-managers', { method: 'POST' });
    } catch(e) {}
...
async function sendPushNotification(title, body, type, orderData) {
  const payload = orderData ? {restName:orderData.restName||'',address:orderData.address||'',total:orderData.total||0,delivery:orderData.delivery||0,payment:orderData.payment||'cash',driverName:orderData.driverName||''} : {title,body};
  try {
    const ctrl=new AbortController(); const t=setTimeout(()=>ctrl.abort(),8000);
    await fetch('/api/notify-managers',{method:'POST',headers:await getAuthHeaders(),body:JSON.stringify(payload),signal:ctrl.signal});
    clearTimeout(t);
  } catch(e){ setTimeout(async()=>{try{await fetch('/api/notify-managers',{method:'POST',headers:await getAuthHeaders(),body:JSON.stringify(payload)});}catch(e2){}},3000); }
}
...
function initDriverApp() {
  ...
  setTimeout(()=>{
    if(Notification.permission==='default'){Notification.requestPermission().then(p=>{if(p==='granted')subscribeFCM();});}
...
function listenToDriverOrders() {
  ...
  const todayStart = (typeof getDayStart==='function'&&currentUser) ? getDayStart() : (()=>{const d=new Date();d.setHours(0,0,0,0);return d;})();
...
function getTodayOrders() {
  const start = (typeof getDayStart==='function'&&currentUser) ? getDayStart() : (()=>{const d=new Date();d.setHours(0,0,0,0);return d;})();
...
async function addOrder() {
  ...
  const _rs=document.getElementById('restSelect'); if(_rs) selectedRest=_rs.value||null;
  if (!selectedRest) { showToast('اختر المطعم أولاً'); return; }
  const submitBtn = document.getElementById('submitOrderBtn') || document.querySelector('.submit-order-btn');
...
async function editOrder(id) {
  ...
  const _rs2=document.getElementById('restSelect'); if(_rs2) _rs2.value=o.restId||''; else selectedRest=o.restId; selectPayment(o.payment); goPage(2); showToast('📝 جاري التعديل...');
}
...
async function addUser(role) {
  ...
  try {
    const res = await fetch('/api/create-user', {method:'POST', headers: await getAuthHeaders(), body: JSON.stringify({name,phone:p,email,pin,role})});
    const data = await res.json();
    if (!res.ok) { if(btn){btn.disabled=false;btn.textContent='إنشاء';} showToast('❌ '+(data.error||'خطأ')); return; }
    await db.collection('users').doc(data.uid).set({uid:data.uid,name,phone:p,email,role,pin,createdAt:firebase.firestore.FieldValue.serverTimestamp(),createdBy:currentUser.uid});
    allDrivers.push({uid:data.uid,name,phone:p,email,role}); renderDriversList(); closeModal();
...
function showDriverDetail(uid) {
  ...
  if (extraBtns) {
    extraBtns.innerHTML = [
      ...
      '<button class="modal-btn cancel" onclick="editDriverFull(\''+uid+'\')" style="width:100%;margin-bottom:8px;background:var(--orange-bg);color:var(--orange);border-color:var(--orange)">✏️ تعديل البيانات</button>',
...
function getDayStart() {
  try {
    ...
    if (s) { const d=new Date(parseInt(s)); const t=new Date(); t.setHours(0,0,0,0); if(d>=t) return d; }
  } catch(e) {}
...
function startNewDay() {
  ...
  showModal('🌅 بداية يوم جديد',
    '<p style="color:var(--text2);font-size:14px;">' + (n>0 ? 'عندك <strong style="color:var(--orange)">'+n+' أوردر</strong> في اليوم الحالي.' : 'هتبدأ يوم جديد من دلوقتي.') + '</p>',
    [{label:'ابدأ',cls:'confirm',action:async()=>{
      localStorage.setItem('nabilpro_daystart_'+currentUser.uid, Date.now().toString());
      try { await db.collection('users').doc(currentUser.uid).update({dayStartAt:firebase.firestore.FieldValue.serverTimestamp()}); } catch(e) {}
...
async function showDriverNotes() {
  ...
  showModal('📬 رسائل من المدير',
    '<div id="notesInboxList"><div class="empty-state"><div class="empty-text">⏳ جاري التحميل...</div></div></div>',
    [{label:'إغلاق',cls:'cancel',action:closeModal}]);
  try {
    const snap = await db.collection('notes').where('driverId','==',currentUser.uid).get();
    ...
    if (!docs.length) { el.innerHTML='<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-text">لا رسائل بعد</div></div>'; return; }
    el.innerHTML = docs.map(doc=>{
      const d=doc.data(); const t=d.createdAt?.toDate?.()??new Date();
      const ts=t.toLocaleDateString('ar-EG',{day:'2-digit',month:'2-digit'})+' '+t.toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'});
      return '<div style="background:var(--bg2);border-radius:12px;padding:14px;margin-bottom:10px;border:1px solid var(--border)"><div style="display:flex;justify-content:space-between;margin-bottom:8px"><span style="font-size:11px;color:var(--text3)">📝 من المدير</span><span style="font-size:11px;color:var(--text3)">'+ts+'</span></div><div style="font-size:14px;line-height:1.8">'+sanitize(d.text||'')+'</div></div>';
    }).join('');
  } catch(e) {
    const el=document.getElementById('notesInboxList');
...
async function checkNewNotes() {
  ...
  try {
    ...
    const badge = document.getElementById('notesCountBadge');
    if (badge) { if(cnt>0){badge.textContent=cnt;badge.style.display='block';}else{badge.style.display='none';} }
  } catch(e) {}
}
...
'<input class="form-field" type="tel" id="ef_pin" placeholder="6 أرقام" maxlength="6" inputmode="numeric" oninput="this.value=this.value.replace(/\D/g,\'\').slice(0,6)"></div>'+
  ...
  showModal('✏️ تعديل — '+sanitize(driver.name||''), body, [
    {label:'💾 حفظ',cls:'confirm',action:async()=>{
      ...
      if(btn){btn.disabled=true;btn.textContent='جاري...';}
      try {
        const upd={name}; if(phone.length>=10) upd.phone=phone.replace(/[^0-9]/g,'');
        const proms=[db.collection('users').doc(uid).update(upd)];
        if(pin.length===6) proms.push(fetch('/api/update-pin',{method:'POST',headers:await getAuthHeaders(),body:JSON.stringify({uid,pin})}).then(r=>r.ok?r.json():r.json().then(e=>{throw new Error(e.error||'خطأ PIN');})));
        await Promise.all(proms);
        driver.name=name; if(phone.length>=10) driver.phone=phone.replace(/[^0-9]/g,'');
        document.getElementById('detailDriverName').textContent=name;
        renderDriversList(); closeModal();
        showToast('✅ تم تحديث '+name+(pin.length===6?' وتغيير الكود':''));
      } catch(e) { if(btn){btn.disabled=false;btn.textContent='💾 حفظ';} showToast('❌ '+(e.message||'خطأ')); }
    }},{label:'إلغاء',cls:'cancel',action:closeModal}]);
  setTimeout(()=>{
    const rb=document.getElementById('ef_role_btn'); const db_btn=document.getElementById('ef_del_btn');
    if(rb) rb.addEventListener('click',()=>quickRoleToggle(uid),{once:true});
    if(db_btn) db_btn.addEventListener('click',()=>quickDeleteDriver(uid),{once:true});
  },50);
}
...
function quickRoleToggle(uid) {
  ...
  closeModal();
  showModal(nr==='manager'?'ترقية':'تحويل','<p style="color:var(--text2);font-size:14px;">'+sanitize(driver.name)+' → '+(nr==='manager'?'مدير':'مندوب')+'؟</p>',
    [{label:'تأكيد',cls:'confirm',action:async()=>{ await db.collection('users').doc(uid).update({role:nr}); driver.role=nr; renderDriversList(); closeModal(); showToast('✅ تم'); }},
     {label:'إلغاء',cls:'cancel',action:closeModal}]);
}
...
function quickDeleteDriver(uid) {
  ...
  showModal('حذف','<p style="color:var(--red);font-size:14px;">حذف "'+sanitize(driver?.name||'')+'"؟</p>',
    [{label:'🗑 حذف',cls:'danger',action:async()=>{ await db.collection('users').doc(uid).delete(); allDrivers=allDrivers.filter(d=>d.uid!==uid); renderDriversList(); closeModal(); closeDriverDetail(); showToast('✅ تم الحذف'); }},
     {label:'إلغاء',cls:'cancel',action:closeModal}]);
}
...
async function checkMonthlyCleanup() {
  try {
    ...
    const snap=await db.collection('orders').where('timestamp','<',firebase.firestore.Timestamp.fromDate(ago)).limit(1).get();
    if(!snap.empty) showModal('🗑 تنظيف شهري','<p style="color:var(--text2);font-size:14px;">فيه أوردرات أقدم من شهر. حذفها هيخلي التطبيق أسرع.</p>',
      [{label:'🗑 حذف',cls:'danger',action:()=>deleteOldOrders(ago)},{label:'بعدين',cls:'cancel',action:closeModal}]);
  } catch(e) {}
}
...
async function deleteOldOrders(before) {
  ...
  try {
    let del=0,snap;
    do { snap=await db.collection('orders').where('timestamp','<',firebase.firestore.Timestamp.fromDate(before)).limit(50).get(); if(snap.empty)break; const b=db.batch(); snap.docs.forEach(d=>b.delete(d.ref)); await b.commit(); del+=snap.docs.length; } while(!snap.empty);
    closeModal(); showToast('✅ تم حذف '+del+' أوردر قديم');
  } catch(e){ if(btn){btn.disabled=false;btn.textContent='🗑 حذف';} showToast('❌ '+e.message); }
}
...
function filterMgrOrders() {
  ...
  const el=document.getElementById('mgrRecentOrders');
  if(!el) return;
  if(!list.length){el.innerHTML='<div class="empty-state"><div class="empty-icon">🔍</div><div class="empty-text">لا نتائج</div></div>';return;}
  el.innerHTML=list.slice(0,30).map(o=>{
    ...
    return '<div class="feed-card '+(v?'visa':'cash')+'"><div class="feed-pay">'+(v?'💳':'💵')+'</div><div class="feed-body"><div class="feed-rest-name">'+sanitize(o.restName||'—')+'</div><div class="feed-driver-info">👤 '+sanitize(o.driverName||'—')+' • 📍 '+sanitize(o.address||'—')+'</div><div class="feed-time-txt">⏰ '+ds+' '+ts2+'</div></div><div class="feed-amt">ج '+(o.delivery||0)+'</div></div>';
  }).join('');
}
...
function showModal(title,bodyHTML,buttons) {
  ...
  buttons.forEach((b,i)=>{ const _b=document.getElementById('mBtn'+i); _b.addEventListener('click',b.action,{once:true}); });
