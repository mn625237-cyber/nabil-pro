...
def schedule_batch_notification():
    """تجميع الإشعارات — لو جم 3 أوردرات في 10 ثواني يبعت إشعار واحد"""
    global _batch_timer
    with _pending_lock:
        if _batch_timer:
            _batch_timer.cancel()
        _batch_timer = threading.Timer(10.0, flush_pending_orders)
        _batch_timer.start()

def flush_pending_orders():
    global _pending_orders, _batch_timer
    with _pending_lock:
    ...
    if len(orders) == 1:
        ...
        drivers = list(set(o["driver"] for o in orders))
        rests = list(set(o["rest"] for o in orders))
        total_delivery = sum(o["delivery"] for o in orders)
        send_fcm(
            f'🛵 {len(orders)} أوردرات جديدة',
            f'{", ".join(drivers[:2])} | {", ".join(rests[:2])}\nإجمالي توصيل: {total_delivery} ج'
        )

# ══════════════════════════════════
# SECURITY — التحقق من Firebase Token
# ══════════════════════════════════
def verify_manager_token(req):
    """التحقق إن المستخدم مدير عبر Firebase ID Token"""
...
def send_fcm_to_user(uid, title, body):
    try:
        ...
        if not token_doc.exists: return
...
def update_pin():
    # تأمين: التحقق من أن المستخدم مدير
    manager_uid = verify_manager_token(request)
    if not manager_uid:
        return jsonify({'error': 'غير مصرح — مديرين فقط'}), 403
    try:
        ...
        print(f'✅ PIN محدث للمستخدم: {uid} بواسطة: {manager_uid}')
